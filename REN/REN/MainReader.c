 ...
 * -------------------------------------------------------------
 */
nonetype bErrorPrint(str fmt, ...);
nonetype displayBuffer(Buffer* ptr_Buffer);
ren_long getFileSize(str fname);
int isNumber(const str ns);
nonetype startReader(str, str, ren_char, int, int);
...
************************************************************
*/
nonetype startReader(str program, str input, ren_char mode, int size, int increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	int loadSize = 0;		/* the size of the file loaded in the buffer */
	ren_char symbol;			/* symbol read from input file */
...
nonetype bErrorPrint(str fmt, ...) {
	...
	(nonetype)vfprintf(stderr, fmt, ap);
	va_end(ap);
